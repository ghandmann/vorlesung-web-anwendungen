...
- [CSS Diner](https://flukeout.github.io): Spaß mit CSS Selektoren
...
# CSS:  Custom Properties aka Variablen

- Häufig wiederholt man Werte (z.B. für Farben) im CSS an vielen stellen
    - Das ist schlecht für die Wartbarkeit
- Mit `CSS Custom Properties` kann man solche Werte einmalig definieren und immer wieder verwenden
- [Dokumentation](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Cascading_variables/Using_custom_properties)
---
...
```css
:root {
    --main-bg-color: #777; /* globale custom properties */
    --main-font-size: 12pt;
}
h1 {
    --my-border-radius: 20px; /* lokales custom property */
...
p {
    ...
    border-radius: var(--my-border-radius); /* ERROR, --main-border-radius ist hier nicht sichtbar */
}
```
...
- Scoping beschreibt die "Sichtbarkeit" von Variablen innerhalb des Quellcodes
- Default: Function-Level-Scope. Alle Variablen sind in der **gesamten** Funktion verfügbar. Bad Practice :-1:
- Gefährlicher Sonderfall:
    - Globale mit `var` deklarierte Variablen werden automagisch zum `window` object im Browsern hinzugefügt!
    - Global verfügbar. 
- Verwenden Sie `let` und `const`, diese aktivieren Block-Level-Scope.
